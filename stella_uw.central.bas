...
, show_err As Boolean)
    ...
    'fix secondary access app
    With load.secondary_access_app
        '2 September 2025, CK: It seems that sometimes, these scripts do not close as they should. _
        As this entire routine is wrapped inside a "On Error Resume Next", what I am doing is opening both _
        scripts and then closing them. The open will fail if the file is already open, but that is okay, as _
        the .CloseCurrentDatabase will succeed.

        .OpenCurrentDatabase load.system_info.system_paths.scripts_path_secondary
        .CloseCurrentDatabase
        .OpenCurrentDatabase load.system_info.system_paths.scripts_path
        .CloseCurrentDatabase
...
err_handler:
    ...
    Resume outro
End Sub
Public Sub backup_deal_tables()
    Const proc_name = "central.backup_deal_tables"
    utilities.call_stack_add_item proc_name
    On Error GoTo err_handler
    If load.is_debugging = True Then On Error GoTo 0
    
    Dim backup_folder_path As String
    Dim data_points As String
    Dim deals_t_eur As String
    Dim deals_t_us As String
    Dim file_path_eur As String
    Dim file_path_us As String
    Dim file_name_eur As String
    Dim file_name_us As String
    Dim fileNum As Long
    Dim i As Integer
    ...
    & ", BuyerDomicile" _
    & ", closing_date, closing_set_received_id, closing_premium_due_date, closing_premium_received_date, ClosingBooked, comments, counsel_fee_amount, counsel_fee_due_date, counsel_fee_received_date" _
    & ", CounselInvoiceHandled, CounselInvoiceReceived, create_date, creating_uw" _
    & ", currency_date, currency_date_manual, currency_rate_eurusd, currency_rate_deal, currency_rate_local" _
    & ", deal_currency, deal_info, deal_name, deal_status_id, drop_end, drop_period" _
    & ", esg_company, esg_country, expected_signing_text" _
    & ", ev" _
    & ", fx_buffer" _
    & ", inception_date, initial_premium_received, insured_registered_country_id, internal_approver, insured_legal_name, insured_navins_code, internal_approver_at_signing, insured_main_region_id" _
    & ", is_folder_created, is_locked, is_repeat_buyer, is_test_deal_id, is_underwritten, IsClosed" _
    & ", key_financials" _
    & ", latest_status_change, lowest_rp_attpoint" _
    & ", max_limit_quoted" _
    & ", nbi_prepper" _
    & ", policy_period_in_months, primary_insurer, primary_or_xs_id, primary_uw, program_limit" _
    & ", quote_due_date, quote_due_time"
    
    data_points = data_points & ", re_quote_info, retention, risk_feel_id, risk_type_id, rr_done" _
    & ", sanction_checks_done, secondary_uw, seller_business_name, seller_legal_name, SellerLegalFirm, SellerDomicile, spa_law, spa_signing_date" _
    & ", signing_invoice_amount, signing_premium_due_date, signing_premium_received_date, SigningBooked, spa_region" _
    & ", sub_risk_type_id" _
    & ", submission_date, submission_limits, submission_notes, surplus_broker_firm, surplus_broker_license_no, surplus_broker_person, stage" _
    & ", target_business_name, target_desc, target_legal_name, target_main_jurisdiction_id, target_super_sector_id, target_sub_sector_id, TargetDomicile" _
    & ", target_naic_code, target_sic_code" _
    & ", total_rp_limit_on_deal, total_rp_premium_on_deal, transaction_style" _
    & ", UltimateBuyer, UltimateSeller, us_producer_no" _
    & ", uw_fee_amount, uw_fee_due_date, uw_fee_received_date, uw_law_firm_id, uw_progress, uw_progress_latest_change, uw_financial_advisor_id" _
    & ", UwCounselPerson1, UwCounselPerson2" _
    & ", vat_home, VDRPassword, VDRReceived" _
    & ", was_quoted_id, workspace_id"
    
    deals_t_eur = "stella_eur.deals_t"
    deals_t_us = "stella_us.deals_t"
    str_date = utilities.generate_sql_date(Date)
    backup_folder_path = load.system_info.system_paths.mysql_backup_path
    file_name_eur = str_date & "_deals_t_eur.csv"
    file_name_us = str_date & "_deals_t_us.csv"
    file_path_eur = backup_folder_path & file_name_eur
    file_path_us = backup_folder_path & file_name_us
    fileNum = FreeFile
    
    'International
    If Dir(file_path_eur) = "" Then
        Open file_path_eur For Output As #fileNum
        
        str_sql = "SELECT " & data_points _
        & " FROM " & deals_t_eur _
        & " WHERE is_deleted = 0"
        ...
        Close #fileNum
    End If
    str_milestone = "end of stella_eur.deals_t"
    
    'Americas
    If Dir(file_path_us) = "" Then
        Open file_path_us For Output As #fileNum
        
        str_sql = "SELECT " & data_points & " FROM " & deals_t_us
        Set rs = utilities.create_adodb_rs(conn, str_sql)
        rs.Open
        ...
        Close #fileNum
    End If
    str_milestone = "end of stella_us.deals_t"
...
err_handler:
    Central.err_handler proc_name, Err.Number, Err.Description, str_sql, "", "", True
    Resume outro
End Sub
Public Sub deal_status_was_changed(ByVal deal_id As Long _
...
, ByVal old_deal_status_id As Long)
    ...
    If update_other_forms = True Then
    ...
    If new_deal_status_id = load.deal_statuses.submission Or old_deal_status_id = load.deal_statuses.NDA Then
    ...
    If utilities.is_old_and_new_deal_status_working(new_deal_status_id, old_deal_status_id) = False Then
        Central.update_currencies deal_id
        Central.folder_check deal_id, new_deal_status_id, old_deal_status_id
    End If
